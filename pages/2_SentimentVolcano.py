...
from utils.negative_pressure_report_generator import generate_pressure_html
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip", delete=False) as zip_temp:
            with zipfile.ZipFile(zip_temp.name, 'w') as zf:
                for _, row in active_alerts.iterrows():
                    ticker = row["ticker"]
                    path = generate_pressure_html(row["ticker"], volcano_df, weekly_df, date_range_str)
